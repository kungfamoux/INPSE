...
export default function ParentMessages() {
  ...
  const filtered = announcements.filter(a => a.audience === "all" || a.audience === "parents");
  ...
  return (
    <div className="space-y-4">
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center"><p className="text-gray-400">No messages.</p></div>
      ) : (
        filtered.map(a => (
          <div key={a.id} className="bg-white rounded-2xl p-5 shadow-sm">
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {a.created_date && <span className="text-xs text-gray-400">{format(new Date(a.created_date), "MMM d")}</span>}
              </div>
            </div>
